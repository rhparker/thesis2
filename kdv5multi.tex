...
In this chapter, we discuss the existence and spectral stability of multi-pulse solutions to KdV5, using the general form derived in \cref{chapter:kdv5general}. We will be able to obtain a instability result, i.e. we can classify which multi-pulses have a positive, real eigenvalue. However, we will only be able to locate purely imaginary eigenvalues to leading order, which does not permit us to obtain a rigorous stability result. We will also discuss the existence of periodic multi-pulses, which resemble multi-pulses but are on a periodic domain. Spectral stability of these periodic multi-pulses remains an open problem, although we present strong numerical results in \cref{chapter:kdv5}.
...
\begin{enumerate}[(i)]
\item There exists a unique $n$-pulse solution $Q_n(x)$ to \cref{genODE} which resembles $n$ consecutive copies of $Q(x)$. 
...
\end{equation}
For the remainder terms $\tilde{Q}_j^\pm(x)$ we have the estimates
