...
\section{Existence of pulses}\label{sec:genexist}

\subsection{Primary pulse solution}\label{sec:primarypulse}

We now address the existence of a primary pulse solution, which is a symmetric homoclinic orbit connecting the rest state equilibrium to itself. In general, the existence of such a solution is unknown, although in specific cases such as KdV5 we do have an existence result. Thus we will take the existence of a symmetric primary pulse solution as a hypothesis.
...
There exist two linearly independent, bounded solutions $\Psi(x)$ and $\Psi^c(x)$ to the adjoint variational equation \eqref{adjvareq2}.
	\begin{enumerate}[(i)]
	...
	\end{equation}
	\end{enumerate}
	Any other bounded solution to \eqref{adjvareq2} is a linear combination of these. Finally
	\[
...
We note that for convenience we have used the notation $Q(x)$ and $\Psi(x)$ in both \cref{sec:genexist} and \cref{sec:genspectrum} to denote the primary pulse and the localized solution to the adjoint variational equation. Since we will be considering the existence and the stability problem separately, this should not cause confusion.
\end{remark}
